...
interface TerminalPlatformApi {
    ...
    fun onSetSimulatedOfflineModeConfiguration(
    ...
    fun onStartCollectPaymentMethod(
        ...
        customerCancellationEnabled: CustomerCancellationApi,
    ...
    private fun onMethodCall(
        ...
        try {
            ...
            when (call.method) {
                ...
                "setSimulatedOfflineModeConfiguration" -> {
                    onSetSimulatedOfflineModeConfiguration((args[0] as List<Any?>).let { SimulatedOfflineModeConfigurationApi.deserialize(it) })
                    result.success(null)
                }
                "getPaymentStatus" -> {
                ...
                "startConfirmPaymentIntent" -> {
                    val res = Result<PaymentIntentApi>(result) { it.serialize() }
                    onStartConfirmPaymentIntent(res, (args[0] as Number).toLong(), args[1] as String, (args[2] as List<Any?>?)?.let { SurchargeConfigurationApi.deserialize(it) })
                }
                ...
                "isTapToPayAccountLinked" -> {
                    ...
                    onIsTapToPayAccountLinked(res, args[0] as String?)
                }
            }
...
enum class CustomerCancellationApi {
    ENABLE,
    DISABLE;
}

data class AmountDetailsApi(
    val surcharge: SurchargeDetailsApi?,
    val tip: TipApi?,
) {
    fun serialize(): List<Any?> {
        return listOf(
...
data class BluetoothConnectionConfigurationApi(
    ...
    val testReaderUpdate: TestReaderUpdateApi?,
): ConnectionConfigurationApi() {
    companion object {
        fun deserialize(
            serialized: List<Any?>,
            ...
            return BluetoothConnectionConfigurationApi(
                ...
                testReaderUpdate = (serialized[2] as List<Any?>?)?.let { TestReaderUpdateApi.deserialize(it) },
            )
        }
    }
}
...
enum class SimulatedOfflineModeApi {
    DISABLED, OFFLINE_IMMEDIATE, OFFLINE_TIMEOUT, OFFLINE_INTERMITTENT;
}

data class SimulatedOfflineModeConfigurationApi(
    val readerOfflineMode: SimulatedOfflineModeApi,
    val sdkOfflineMode: SimulatedOfflineModeApi,
) {
    companion object {
        fun deserialize(
            serialized: List<Any?>,
            ...
            return SimulatedOfflineModeConfigurationApi(
                readerOfflineMode = (serialized[0] as Int).let { SimulatedOfflineModeApi.values()[it] },
                sdkOfflineMode = (serialized[1] as Int).let { SimulatedOfflineModeApi.values()[it] },
            )
        }
    }
}
...
data class SurchargeConfigurationApi(
    val amount: Long,
    val consent: SurchargeConsentApi?,
) {
    companion object {
        fun deserialize(
            serialized: List<Any?>,
            ...
            return SurchargeConfigurationApi(
                amount = (serialized[0] as Number).toLong(),
                consent = (serialized[1] as List<Any?>?)?.let { SurchargeConsentApi.deserialize(it) },
            )
        }
    }
}
...
data class TestReaderUpdateApi(
    val components: List<UpdateComponentApi>?,
    val updateType: TestReaderUpdateTypeApi,
) {
    companion object {
        fun deserialize(
            serialized: List<Any?>,
            ...
            return TestReaderUpdateApi(
                components = (serialized[0] as List<Any?>?)?.map { (it as Int).let { UpdateComponentApi.values()[it] } },
                updateType = (serialized[1] as Int).let { TestReaderUpdateTypeApi.values()[it] },
            )
        }
    }
}

enum class TestReaderUpdateTypeApi {
    AVAILABLE, REQUIRED, REQUIRED_OFFLINE, LOW_BATTERY, LOW_BATTERY_SUCCEED_CONNECT;
}
